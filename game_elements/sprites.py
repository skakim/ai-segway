...
class GameObject(BaseSprite):
    def __init__(self, type, batch, space, group):
        if type == "wheel":
            ...
            self.type = "wheel"
            self.obj_space = PymunkSpace().body_space(space, (constants.W_WIDTH/2,constants.W_HEIGHT/2), 41, 70)
        else:
            image = './resources/smiley.png'
            self.type = "smiley"
            self.obj_space = PymunkSpace().body_space(space,(constants.W_WIDTH/2, constants.W_HEIGHT/2 + constants.ROD_LENGTH), 60, 35)
    ...
    def reset(self):
        ...
        self.obj_space.body.position.x = constants.W_WIDTH/2
        if self.type == "wheel":
            self.obj_space.body.position.y = constants.W_HEIGHT/2
        else:
            self.obj_space.body.position.y = constants.W_HEIGHT/2 + constants.ROD_LENGTH
            self.obj_space.body.apply_impulse(j = (random.randint(-10000, 10000), 0))

    def randomize(self, offset = None):
        if offset is None:
            offset = random.randint(-constants.W_WIDTH/2 + 30, constants.W_WIDTH/2 - 30)
